...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h4 section-title mb-0">تعديل المنتج #{{ $product->id }}</h1>
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-8">
                    ...
                    @if($product->image_path)
                        <div class="small-muted mt-1">الصورة الحالية: {{ $product->image_path }}</div>
                    @endif
                </div>
            </div>
            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" value="1" id="is_active" name="is_active" @checked(old('is_active', $product->is_active))>
                <label class="form-check-label" for="is_active">
                    منتج مفعل يظهر في المتجر
                </label>
            </div>
            <button class="btn btn-dark" type="submit">تحديث المنتج</button>
        </form>
    </div>
@endsection
